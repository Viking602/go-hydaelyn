...
func LoadCase(path string) (eval.EvalCase, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		return eval.EvalCase{}, fmt.Errorf("read eval case: %w", err)
	}
	var c eval.EvalCase
	if err := json.Unmarshal(data, &c); err != nil {
		return eval.EvalCase{}, fmt.Errorf("decode eval case: %w", err)
	}
