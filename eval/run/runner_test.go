...
func TestDeterministicEvalRunner(t *testing.T) {
  ...
  {"id":"policy-001","date":"2026-04-01T00:00:00Z","text":"retain customer data for 30 days"}
	...
	if run.Status != eval.EvalRunStatusCompleted {
	...
	for _, name := range []string{"events.json", "replay.json", "state.final.json", "state.replayed.json", "answer.txt", "score.json", "quality.score.json", "eval.report.json", "tool_calls.jsonl", "model_events.jsonl", "summary.md", "manifest.json", "run.json"} {
