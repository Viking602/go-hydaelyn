...
func (r *Runner) RunCaseDirectory(ctx context.Context, root string) (*SuiteRun, error) {
	casePaths, err := cases.DiscoverCasePaths(root)
	if err != nil {
		return nil, err
	}
...
func (r *Runner) RunSuite(ctx context.Context, name, root string, casePaths []string) (*SuiteRun, error) {
	...
	scores := make([]eval.ScorePayload, 0, len(casePaths))
	passed := 0
	...
	aggregateScore := eval.AggregateScores(suiteID, suiteName, scores)
	capabilityReport := eval.GenerateCapabilityReport(aggregateScore)
	releaseDecision := eval.ReleaseDecisionNoGo
	pass := false
	if aggregateScore != nil {
		pass = aggregateScore.Pass
		if pass {
			releaseDecision = eval.EvaluateReleaseGate(aggregateScore)
		}
...
func (r *Runner) runSuiteCase(ctx context.Context, casePath string) (SuiteCaseResult, eval.ScorePayload) {
	...
	if err != nil {
		...
		return SuiteCaseResult{
			...
			Suite:    meta.Suite,
			CasePath: casePath,
			Status:   eval.EvalRunStatusFailed,
			Pass:     false,
			Level:    score.Level,
			Error:    err.Error(),
		}, score
...
func failedSuiteScore(meta suiteCaseMeta, err error) eval.ScorePayload {
	score := eval.ScorePayload{
		SchemaVersion:    eval.ScorePayloadSchemaVersion,
		RunID:            firstNonEmptyString(meta.CaseID, "failed-case"),
		CaseID:           meta.CaseID,
		Suite:            meta.Suite,
		Pass:             false,
		ReplayConsistent: false,
		RuntimeMetrics: &eval.ScoreRuntimeMetrics{
			TaskCompletionRate:  0,
			BlockingFailureRate: 1,
			RetrySuccessRate:    0,
		},
		Failures: []eval.ScoreFailure{{
			Code:     "run.execution_failed",
			Message:  err.Error(),
	...
	score.Level = eval.ApplyHardDowngradeRules(&score)
	return score
}

func loadSuiteScore(run *eval.EvalRun) (eval.ScorePayload, error) {
	if run == nil || run.ScoreRef == nil || strings.TrimSpace(run.ScoreRef.Path) == "" {
		return eval.ScorePayload{}, fmt.Errorf("eval run score ref is missing")
	}
	data, err := os.ReadFile(run.ScoreRef.Path)
	if err != nil {
		return eval.ScorePayload{}, fmt.Errorf("read score artifact: %w", err)
	}
	var score eval.ScorePayload
	if err := json.Unmarshal(data, &score); err != nil {
		return eval.ScorePayload{}, fmt.Errorf("decode score artifact: %w", err)
	}
	return score, nil
}
...
func refPath(ref *eval.EvalRunRef) string {
	if ref == nil {
		return ""
	}
	return ref.Path
...
func scoreDirectory(run *eval.EvalRun) string {
