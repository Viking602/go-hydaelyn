-- storage/sqlite reference implementation schema (v0.8.0).
-- Single-file canonical DDL; MigrationsFS() exposes it for external tools.
-- Every store keeps the full value as JSON in `payload`; columns outside
-- payload exist only when the contract requires querying or atomicity
-- (lease CAS version, event sequence, FIFO created_at, status filtering).
