...
func TestRecoverDoesNotRedispatchRequiresReconcileAttempt(t *testing.T) {
	ctx := context.Background()
	rt := NewMemoryRuntime()
	run := mustStartRun(ctx, t, rt, "run-reconcile-no-redispatch")
	task := mustCreateTask(ctx, t, rt, CreateTaskCommand{
		RunID:        run.ID,
		TaskID:       "action",
		OwnerAgentID: "agent-a",
	...
	attempt.Status = ActionAttemptUnknown
	...
	if err := uow.Events().AppendEvent(ctx, Event{
		RunID:      run.ID,
		TaskID:     task.ID,
		Type:       EventActionAttemptUpdated,
	...
	if got := mustLoadTask(ctx, t, rt, run.ID, task.ID); got.Status != TaskStatusReconcileRequired {
		t.Fatalf("recovered task status = %q, want reconcile_required (must not redispatch)", got.Status)
	}
	if got := mustLoadEnvelope(ctx, t, rt, envelope.ID); got.Status == "pending" && got.TaskVersion != task.Version {
		t.Fatalf("recovery redispatched reconcile-required attempt: %#v", got)
	}
	if got, err := rt.Run(ctx, run.ID); err != nil || got.Status != RunStatusReconcileRequired {
		t.Fatalf("recovered run = %#v err=%v, want reconcile_required", got, err)
	}
...
func TestResumeTokens_ReadsConfiguredStoreProvider(t *testing.T) {
	...
	got, err := rt.ResumeTokens(ctx)
	if err != nil {
		t.Fatalf("ResumeTokens() error = %v", err)
	}
	if len(got) != 1 {
		t.Fatalf("ResumeTokens() = %#v, want the token persisted on the external provider", got)
	}
	if got[token.TokenID].RunID != token.RunID {
		t.Fatalf("ResumeTokens()[%s] = %#v, want run %s", token.TokenID, got[token.TokenID], token.RunID)
	}
}
