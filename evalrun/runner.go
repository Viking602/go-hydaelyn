...
func (r *Runner) Run(ctx context.Context, casePath string) (_ *evaluation.EvalRun, err error) {
	...
	score, err := evaluation.ScoreCase(evalRun, canonicalEvents, evalCase)
	if err != nil {
		return nil, err
	}
	...
	evaluationReportRef := manifestRef(outputDir, "evaluation-report", "evaluation.report.json")
	...
	evalRun.TraceRefs = &evaluation.EvalRunTraceRefs{Events: eventsRef, ModelEvents: modelEventsRef}
	evalRun.ArtifactRefs = &evaluation.EvalRunArtifactRefs{
		Events:           eventsRef,
		Replay:           replayRef,
		Answer:           answerRef,
...
func resolveWorkerProfiles(profiles evaluation.EvalCaseProfiles) []string {
...
func buildEvaluationCorpus(docs map[string]evalcase.CorpusDocument, evalCase evaluation.EvalCase) evaluation.Corpus {
	if len(docs) == 0 {
		return evaluation.Corpus{}
	}
	items := make([]evaluation.CorpusDocument, 0, len(docs))
	if evalCase.Fixtures != nil && len(evalCase.Fixtures.CorpusIDs) > 0 {
		for _, id := range evalCase.Fixtures.CorpusIDs {
			doc, ok := docs[id]
			if !ok {
			...
			items = append(items, evaluation.CorpusDocument{ID: doc.ID, Date: doc.Date, Text: doc.Text})
		}
	} else {
		ids := make([]string, 0, len(docs))
		for id := range docs {
	...
	return evaluation.Corpus{Documents: items}
}

func deterministicQualityMetrics(answer string, corpus evaluation.Corpus, evalCase evaluation.EvalCase) *evaluation.ScoreQualityMetrics {
	expected := evalCase.Expected
	if strings.TrimSpace(answer) == "" && expected == nil && len(corpus.Documents) == 0 {
		return nil
	}
	metrics := &evaluation.ScoreQualityMetrics{}
	hasSignal := false
	...
	refusal := evaluation.DetectRefusal(answer)
	if len(corpus.Documents) == 0 {
		...
		groundedness := evaluation.CheckGroundedness(answer, corpus)
		...
		citations := evaluation.ValidateCitations(answer, corpus)
...
func expectationScore(answer string, expected evaluation.EvalCaseExpected) float64 {
...
func citationIDsFromAnswer(answer string) []string {
	corpus := evaluation.Corpus{Documents: []evaluation.CorpusDocument{}}
	return evaluation.ValidateCitations(answer, corpus).Citations
}
...
func aggregateUsage(state team.RunState) *evaluation.EvalRunUsage {
...
func buildEvaluationArtifact(evalCase evaluation.EvalCase, report evaluation.Report, qualityMetrics *evaluation.ScoreQualityMetrics, score *evaluation.ScorePayload) map[string]any {
...
func persistArtifacts(
	...
	entries := []struct {
		...
		{id: "replay", kind: evaluation.ArtifactManifestKindReplayState, fileName: "replay.json", contentType: "application/json", value: replayedState},
		{id: "state-final", kind: evaluation.ArtifactManifestKindReplayState, fileName: "state.final.json", contentType: "application/json", value: finalState},
		{id: "state-replayed", kind: evaluation.ArtifactManifestKindReplayState, fileName: "state.replayed.json", contentType: "application/json", value: replayedState},
		{id: "evaluation-report", kind: evaluation.ArtifactManifestKindEvaluationReport, fileName: "evaluation.report.json", contentType: "application/json", value: evaluationArtifact},
		{id: "score", kind: evaluation.ArtifactManifestKindScore, fileName: "score.json", contentType: "application/json", value: score},
		{id: "quality-score", kind: evaluation.ArtifactManifestKindScore, fileName: "quality.score.json", contentType: "application/json", value: score},
	}
	...
	artifacts = append(artifacts, artifactRecord{ID: "model-events", Kind: evaluation.ArtifactManifestKindModelEvents, FileName: "model_events.jsonl", ContentType: "application/x-ndjson", Content: modelEventBytes})
	...
	artifacts = append(artifacts, artifactRecord{ID: "tool-calls", Kind: evaluation.ArtifactManifestKindToolCalls, FileName: "tool_calls.jsonl", ContentType: "application/x-ndjson", Content: toolCallBytes})
	summary := []byte("# " + caseID + "\n\n" + answer + "\n")
