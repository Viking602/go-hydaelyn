...
- ADR-012 (Storage contract, new in v0.8.0) — supports Principles 4 and 5. Records the Position C stance: the framework owns the `StoreProvider` contract and the contract test suite; `storage/{sqlite,mysql,postgres}` are reference implementations that exist as starting points for forking; production teams are expected to write their own provider against their data stack (ent / gorm / DBA-controlled DDL). The framework does not commit to operating any reference implementation at production scale; the contract test suite is what validates production providers
