...
func TestPack_ToolsMatchCapabilities(t *testing.T) {
	...
	for _, manifest := range customersupport.Pack.Capabilities {
	...
	for _, agent := range customersupport.Pack.Agents {
	...
	if customersupport.Pack.Name != customersupport.PackName {
		t.Fatalf("pack name %q != PackName %q", customersupport.Pack.Name, customersupport.PackName)
	}
}
